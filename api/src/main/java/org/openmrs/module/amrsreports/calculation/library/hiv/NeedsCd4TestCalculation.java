...
import org.openmrs.module.amrsreports.calculation.CoreUtils;
...
public class NeedsCd4TestCalculation extends BaseEmrCalculation implements PatientFlagCalculation {
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues, PatientCalculationContext context) {
		...
		CalculationResultMap lastObsCount = Calculations.lastObs(getConcept(MohEvaluableNameConstants.ADMITTED_TO_HOSPITAL/*Dictionary.CD4_COUNT*/), cohort, context);
		CalculationResultMap lastObsPercent = Calculations.lastObs(getConcept(MohEvaluableNameConstants.ADMITTED_TO_HOSPITAL/*Dictionary.CD4_PERCENT*/), cohort, context);
		...
		for (Integer ptId : cohort) {
			...
			if (inHivProgram.contains(ptId)) {
				...
				if (lastResultDate == null || (daysSince(lastResultDate, context) > 10 /*EmrConstants.NEEDS_CD4_COUNT_AFTER_DAYS*/)) {
					needsCD4 = true;
				}
			}
