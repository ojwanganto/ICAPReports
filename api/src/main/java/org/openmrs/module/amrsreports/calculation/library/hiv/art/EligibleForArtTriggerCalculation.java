...
public class EligibleForArtTriggerCalculation extends BaseEmrCalculation {
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		CalculationResultMap confirmedPositives = Calculations.allObs(getConcept("2332"/*Dictionary.DATE_OF_HIV_DIAGNOSIS*/), cohort, context);
		CalculationResultMap whoStages = Calculations.allObs(getConcept("2434"/*Dictionary.CURRENT_WHO_STAGE*/), cohort, context);
		CalculationResultMap cdCounts = Calculations.allObs(getConcept("65656"/*Dictionary.CD4_COUNT*/), cohort, context);
		CalculationResultMap cdPercents = Calculations.allObs(getConcept("4343"/*Dictionary.CD4_PERCENT*/), cohort, context);
	...
	private List<Obs> extractTriggers(List<Obs> obss) {
		...
		for (Obs o : obss) {
			...
			if (o.getConcept().equals(getConcept("4343"/*Dictionary.DATE_OF_HIV_DIAGNOSIS*/))) {
			...
			else if (o.getConcept().equals(getConcept("65656"/*Dictionary.CURRENT_WHO_STAGE*/))) {
			...
			else if (o.getConcept().equals(getConcept("7676"/*Dictionary.CD4_COUNT*/))) {
			...
			else if (o.getConcept().equals(getConcept("776"/*Dictionary.CD4_PERCENT*/))) {
	...
	private Obs findEarliestTrigger(List<Obs> triggers) {
		...
		for (Obs trigger : triggers) {
			...
			Date triggerDate = trigger.getConcept().equals(getConcept("656565"/*Dictionary.DATE_OF_HIV_DIAGNOSIS*/))
