 ...
 */
@Handler(supports = ICAP2EligibilityForARTDataDefinition.class, order = 50)
public class ICAP2EligibilityForARTDataEvaluator extends BatchedExecutionDataEvaluator<ObsRepresentation> {

	private Log log = LogFactory.getLog(getClass());

	private Map<Integer, Object> artStartDates;

	private ICAP2EligibilityForARTDataDefinition definition;
	...
	protected PersonDataDefinition setDefinition(PersonDataDefinition def) {
		definition = (ICAP2EligibilityForARTDataDefinition) def;
		return definition;
	}
	...
	protected boolean doBefore(EvaluationContext context, EvaluatedPersonData c, Cohort cohort) {
		...
		try {
			otherColumn = Context.getService(PersonDataService.class).evaluate(new ICAPArvStartDateDataDefinition(), context);
