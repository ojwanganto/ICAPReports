 ...
 */
@Handler(supports = ICAPCD4CountDataDefinition.class, order = 50)
public class ICAPCD4COUNTDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context) throws EvaluationException {
        ...
        String sql = "select person_id, value_numeric " +
                " 	from obs" +
                " 	where" +
        ...
        ListMap<Integer, Double> mappedCD4Concepts = makeCD4MapFromSQL(sql, m);

        for (Integer memberId : context.getBaseCohort().getMemberIds()) {
            Double cd4 = safeFind(mappedCD4Concepts, memberId);

            c.addData(memberId, cd4);
        }

		return c;
	}

    protected ListMap<Integer, Double> makeCD4MapFromSQL(String sql, Map<String, Object> substitutions) {
        List<Object> data = Context.getService(MohCoreService.class).executeSqlQuery(sql, substitutions);

        return makeDatesMap(data);
    }

    protected ListMap<Integer, Double> makeDatesMap(List<Object> data) {
        ListMap<Integer, Double> cd4ListMap = new ListMap<Integer, Double>();
        for (Object o : data) {
            Object[] parts = (Object[]) o;
            if (parts.length == 2) {
                Integer pId = (Integer) parts[0];
                Double cd4 = (Double) parts[1];
                cd4ListMap.putInList(pId, cd4);
            }
        }
    ...
    protected Double safeFind(final ListMap<Integer, Double> map, final Integer key) {
