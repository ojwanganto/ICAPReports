...
import org.openmrs.module.amrsreports.reporting.data.ICAPObsNearestARVStartDateDataDefinition;
 ...
 */
@Handler(supports = ICAPObsNearestARVStartDateDataDefinition.class, order = 50)
public class ICAPObsNearestARVStartDateDataEvaluator extends BatchedExecutionDataEvaluator<ObsRepresentation> {

	private ICAPObsNearestARVStartDateDataDefinition definition;
	...
	protected PersonDataDefinition setDefinition(PersonDataDefinition def) {
		definition = (ICAPObsNearestARVStartDateDataDefinition) def;
		return definition;
	}
	...
	protected boolean doBefore(EvaluationContext context, EvaluatedPersonData c, Cohort cohort) {
		...
		if (definition.getAgeLimit() != null) {
            ...
            ICAPDateARTStartedDataDefinition artStarted = new ICAPDateARTStartedDataDefinition();
			MappedData<ICAPDateARTStartedDataDefinition> mappedDef = new MappedData<ICAPDateARTStartedDataDefinition>();
			...
			for (Integer personId : artStartAges.keySet()) {
				...
				if (age!=null && age.getFullYears() > definition.getAgeLimit())
...
*/
	...
	protected String getHQL() {
		return "select new map(" +
				...
				"		o.valueNumeric as valueNumeric," +
				"		o.obsDatetime as obsDatetime," +
				"		(select ABS(DATEDIFF(o.obsDatetime, o2.valueDatetime )) from Obs o2 WHERE o2.concept.conceptId=159599 and o2.personId=o.personId)  as order)" +
				" FROM Obs AS o " +
				" WHERE" +
				"   o.voided = false" +
				"	AND o.personId in (:personIds) " +
				"	AND o.concept.conceptId in (:conceptIds)"; /*+
				"	AND o.obsDatetime BETWEEN date_add((select o2.valueDatetime from Obs o2 WHERE o2.concept.conceptId=159599), INTERVAL -21 DAY) AND date_add((select o3.valueDatetime from Obs o3 WHERE o3.concept.conceptId=159599), INTERVAL 7 DAY)";
*/
        ...
        /*return "select new map(" +
                ...
                "		o.valueNumeric as valueNumeric," +
                "		o.obsDatetime as obsDatetime," +
                "		ABS(DATEDIFF(o.obsDatetime, select valueDatetime from Obs o WHERE o.concept.conceptId=159599)) as order)" +
                " FROM Obs AS o " +
                " WHERE" +
                "   o.voided = false" +
                "	AND o.personId in (:personIds) " +
                "	AND o.concept.conceptId in (:conceptIds)" +
                "	AND o.obsDatetime BETWEEN SUBDATE((select valueDatetime from Obs o WHERE o.concept.conceptId=159599), 21) AND ADDDATE((select valueDatetime from Obs o WHERE o.concept.conceptId=159599), 7)";
*/
