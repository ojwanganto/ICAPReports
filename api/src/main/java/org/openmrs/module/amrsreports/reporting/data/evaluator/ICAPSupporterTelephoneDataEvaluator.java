...
import org.openmrs.module.amrsreports.reporting.data.ICAPSupporterTelephoneDataDefinition;
 ...
 */
@Handler(supports = ICAPSupporterTelephoneDataDefinition.class, order = 50)
public class ICAPSupporterTelephoneDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context) throws EvaluationException {
        ...
        ListMap<Integer, String> mappedPhoneNumbers = makePhoneNOMapFromSQL(sql, m);

        for (Integer memberId : context.getBaseCohort().getMemberIds()) {
            String phoneNumbers = safeFind(mappedPhoneNumbers, memberId);

            c.addData(memberId, phoneNumbers);
        }
